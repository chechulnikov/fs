...
~ 1 ГБ
```
Однако, даже такой небольшой объём требует bitmap размером в ~ 1 МБ. Искать линейно при каждой аллокации выходит слишком дорого, поэтому поиск осуществляется не по самому bitmap'у, а по дереву отрезков следующего вида
