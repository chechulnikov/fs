...
### Индексы
Реализован один индекс: [B+-дерево](https://en.wikipedia.org/wiki/B%2B_tree) по именам файлов, чтобы поиск файла занимал логарифмическое время. Узлы дерева хранятся в виде индексных блоков. В листьях хранятся соответствующие номера FMB.<br/>
